...
import React, { useState, useEffect, useRef } from 'react'; // Removed Fragment as it's not used directly
...
import {
  ...
  FileText, Menu as MenuIcon, X as XIcon, ChevronDown, Settings, BarChart2, UserCog, Plus // Added Settings, BarChart2, UserCog, Plus
} from 'lucide-react';
...
export function Navbar({ session, profile }: NavbarProps) {
  ...
  const employerNavItems = [
    ...
    { path: '/messages', icon: MessageCircle, label: 'Meddelanden', description: 'Kommunicera med personal' },
  ...
  const employeeNavItems = [
    ...
    { path: '/messages', icon: MessageCircle, label: 'Meddelanden', description: 'Kommunicera med arbetsgivare' },
  ...
  <span className="text-xl font-bold text-primary-700 font-display">Farmispoolen</span>
          ...
          <div className={`flex items-center ${isAuthenticated ? 'ml-auto pl-4' : 'md:ml-auto'} space-x-2`}>
            ...
            {isAuthenticated && (
              <Link
                to="/messages"
                className="hidden md:flex p-2 text-primary-600 hover:text-primary-700 rounded-full hover:bg-primary-50 transition-colors" /* UPDATED colors */
                title="Meddelanden"
              >
