...
export function ShiftDetailsModal({
  ...
  const handleOpenEmployerModal = (e: React.MouseEvent) => {
    ...
    const handleReportSick = async () => {
        ...
        if (result.error) {
            toast.error(`Fel: ${result.error}`, { id: toastId });
        } else {
            toast.success("Sjukanmälan har registrerats. Passet har återpublicerats.", { id: toastId });

          // --- NOTIFICATION LOGIC ADDED HERE ---
          // 2. Trigger the email notification to all employers.
          // We fire and forget, so the UI doesn't wait for this.
  ...
  body: JSON.stringify({
    ...
    payload: {
      ...
      // We add the specific employer's ID here
      employerId: shift.employer_id 
    }
  }),
}).catch(e => console.error("Failed to trigger sick report email:", e));
  ...
  const totalRate = (shift.hourly_rate || 0) + (shift.urgent_pay_adjustment || 0);
    ...
    const isAssignedToThisUser = shift.assigned_applicant_id === currentUserId;
    const canReportSick = isAssignedToThisUser && shift.status === 'filled' && !isShiftPast;
                ...
                <Timer size={16} className="mr-2"/> {countdown}
                    ...
                    <div className="p-4 sm:p-6 overflow-y-auto flex-grow space-y-4">
                        ...
                        <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                            <div className="flex items-center"><Briefcase className="icon-style" /><span>Roll: <span className="font-medium">{shift.required_role || 'N/A'}</span></span></div>
